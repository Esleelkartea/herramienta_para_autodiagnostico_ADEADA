		...
		<tr>
			...
			<td id="LoginWithAjax_Info">
				<?php
					...
					//Logout URL
					?>
					<a id="wp-logout" href="<?php echo wp_logout_url(get_permalink()) ?>"><?php echo __( 'Log Out', 'login-with-ajax' ) ?></a><br />
					<?php
					//Blog Admin
					if( !empty($wpmu_version) || $user_level > 8 ) {
						?>
						<a href="<?php bloginfo('wpurl') ?>/wp-admin/"><?php echo __( 'blog admin', 'login-with-ajax' ) ?></a>
						<?php
					}
				?>
			</td>
